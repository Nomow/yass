...
def plot_clustering_template(fig, grid, ax_t, gen, N, wf, idx_recovered, CONFIG, 
        ...
        ax_t.plot(CONFIG.geom[:,0]*4+
                  np.arange(-wf_mean.shape[0]//2,wf_mean.shape[0]//2,1)[:,np.newaxis], 
                  CONFIG.geom[:,1] + wf_mean[:,:]*scale, c=sorted_colors[N%100], 
                  alpha=min(max(0.4, idx_recovered.shape[0]/1000.),1))

        # plot feature channels
        for i in feat_chans:
             ax_t.scatter(CONFIG.geom[i,0]*4+gen, CONFIG.geom[i,1]+N, 
                                                    s = 30, 
...
#         rhat = rhat/np.sum(rhat,axis =1 ,keepdims = True)
        ...
        for clust in clusters:
            ...
            patch_j = mpatches.Patch(color = sorted_colors[clust%100], 
                label = "size = {}, stability = {}".format(sizes[i], stability[i]))
            labels.append(patch_j)
            i+= 1
        
        # make list of colors; this could be done simpler
        if split_type =='post_removal':
            for i, clust in enumerate(clusters):
                idx = assignment2 == clust
                if clust == -1:
                    ax.scatter(pca_wf[idx,0], pca_wf[idx,1], c = 'lightsteelblue', marker = 'x', edgecolor = 'k', s= 100, alpha = 0.7)
                else:
                    ax.scatter(pca_wf[idx,0], pca_wf[idx,1], c = sorted_colors[clust], edgecolor = 'k', alpha = 0.1)
            ...
            if pca_wf.shape[1]>1:
                ax.scatter(pca_wf[:,0], pca_wf[:,1], 
                    c = colors[assignment2.astype(int)], edgecolor = 'k',alpha=0.1)
...
def run_cluster_features_chunks(spike_index_clear, spike_index_all,
    ...
    standardized_filename = os.path.join(CONFIG.data.root_folder, out_dir,
                                         'standarized.bin')
    standardized_filename = '/ssd/data/Nishchal/yass/src/data/sept_3/standarized.bin'
...
def cluster_channels_chunks_args(data_in):
    ...
    if os.path.exists(filename_postclustering)==False: 
        ...
        if plotting:
            ...
            ax_t = fig.add_subplot(grid[35:, 10:])
        else:
            fig = []
            grid = []
            x = []
        ...
        RRR3_noregress_recovery_dynamic_features_new_prime(channel, 
        ...
        if plotting: 
            ...
            for i in range(CONFIG.recordings.n_channels):
                ax_t.text(CONFIG.geom[i,0]*4, CONFIG.geom[i,1], str(i), alpha=0.4, 
                                                                fontsize=30)
                # fill bewteen 2SUs on each channel
                ax_t.fill_between(CONFIG.geom[i,0]*4 + np.arange(61),
                    -scale + CONFIG.geom[i,1], scale + CONFIG.geom[i,1], 
                    color='black', alpha=0.05)
                
            # plot max chan with big red dot                
            ax_t.scatter(CONFIG.geom[channel,0]*4, CONFIG.geom[channel,1], s = 2000, 
                                                    color = 'red')
...
#         for channel in [0]:
    ...
    np.save(chunk_dir + '/temps_align.npy', templates)
    np.save(chunk_dir + '/spike_times_align.npy', spike_indexes)
...
# def RRR3_noregress_recovery_dynamic_features_new_prime(channel, wf, sic, gen, fig, 
#          grid, x, ax_t, triageflag, alignflag, plotting, n_feat_chans, 
#          n_dim_pca, wf_start, wf_end, mfm_threshold, CONFIG, 
#          upsample_factor, nshifts, assignment_global, spike_index, 
#          scale, knn_triage_threshold, deconv_flag, templates, 
...
#     ## knn triage outliars; e.g. remove 2%
#     if True:
#         idx_keep_triage = knn_triage(90, pca_wf)
#         idx_keep_triage = np.where(idx_keep_triage==1)[0]
#         if verbose:
#             print("chan "+str(channel)+' gen: '+str(gen) + 
#                   " triaged, remaining spikes "+ 
...
#         pca_wf = pca_wf[idx_subsampled]
#     else:
#         idx_subsampled = np.arange(pca_wf.shape[0])
...
#     else:
#         vbParam2, assignment2 = vbParam, assignment
#         idx_recovered = idx_subsampled
#     if verbose:
#         print(idx_recovered.size, idx_keep.size, assignment2.size)
#         print ("chan "+ str(channel)+' gen: '+str(gen)+" - recovered ",
#                                             str(idx_recovered.shape[0]))
...
#                     print("chan "+str(channel)+' gen: '+str(gen)+
#                         " reclustering stable cluster"+ 
#                         str(wf[idx_keep][idx_recovered][idx].shape))
                
#                 triageflag = False
#                 alignflag = True
#                 RRR3_noregress_recovery_dynamic_features_new_prime(channel, wf[idx_keep][idx_recovered][idx], 
#                      sic[idx_keep][idx_recovered][idx], gen+1, fig, grid, x, ax_t, triageflag, alignflag, 
#                      plotting, n_feat_chans, n_dim_pca, wf_start, wf_end, 
#                      mfm_threshold,  CONFIG, upsample_factor, nshifts, 
#                      assignment_global, spike_index, scale, knn_triage_threshold, 
...
#                 G = nx.DiGraph()
#                 for i in np.unique(assignment2[idx_recovered]):
#                     G.add_node(i)
#                 for i, j in zip(row,column):
#                     G.add_edge(i, j, weight = maha[i,j])

#                 assignment3 = assignment2[idx_recovered].copy()
#                 rhat = np.zeros([assignment2[idx_recovered].size,0])
#                 stability = []
#                 for i, units in enumerate(nx.strongly_connected_components(G)):
#                     print(list(units))
#                     idx =  np.in1d(assignment2[idx_recovered], list(units))
#                     assignment3[idx] = i
#                     if len(units) == 1:
#                         print(vbParam2.rhat[idx_recovered][:, list(units)].shape)
#                         rhat = np.concatenate([rhat,vbParam2.rhat[idx_recovered][:, list(units)]], axis = 1)
#                     else:                       
#                         rhat = np.concatenate([rhat,vbParam2.rhat[idx_recovered][:, list(units)].sum(1, keepdims = True)], axis = 1)
#                     mask = rhat[:,-1] > 0
#                     stability.append(np.average(mask * rhat[:,-1], axis = 0, weights = mask))
#                 vbParam3 = vbPar(rhat)
#                 if np.unique(assignment3).size == 1:
#                     maha_thresh = maha_thresh * 0.90
#                 else:
#                     flag = False
...
#             if verbose:
#                 print("chan "+str(channel)+' gen: '+str(gen)+ 
#                                 " no stable clusters, connected_components_split "+
#                                 str(wf[idx_keep][idx_recovered].shape), stability)
...
#     print("time elapsed feat_channel = :", time.time() - start)
#     start = time.time()
    # featurize using latest alg
...
#     print("time elapsed featurization= :", time.time() - start)
#     start = time.time()
    ### idx_keep is integer
...
#     print("time elapsed  garbage= :", time.time() - start)
#     start = time.time()
    vbParam, assignment = run_mfm3(pca_wf, CONFIG)
    
#     print("time elapsed  clustering= :", time.time() - start)
#     start = time.time()
#     vbParam.rhat = np.concatenate([vbParam.rhat, np.zeros([vbParam.rhat.shape[0],1])], axis = 1)
#     vbParam.Vhat = np.concatenate([vbParam.rhat, np.zeros([vbParam.rhat.shape[0],1])], axis = 1)
...
#     else:
#         vbParam2, assignment2 = vbParam, assignment
#         idx_recovered = np.arange(assignment2.size, dtype = int)
    
#     print("time elapsed recovery = :", time.time() - start)
#     start = time.time()
#     ellipses = ret_ellipse(0,1,vbParam2, 1, sorted_colors)
    
#     if plotting and gen<15:
#         print(vbParam2.rhat.shape)
...
#     med_eig = np.median(eigenvalues)
    
#     weight_percent = 3
#     K = eigenvalues.size
#     if eigenvalues.size%2 == 0:
#         sorted_eig = np.argsort(eigenvalues)
#         weight = (vbParam.nuhat[sorted_eig[[int(K/2), int(K/2)-1]]].sum()/2 - 5) * weight_percent
#     else:
#         which_clust = np.where(eigenvalues == med_eig)[0]
#         weight = (vbParam.nuhat[which_clust]-5).sum()*weight_percent
...
#     print("triaged :", idx_recovered.size)
...
#     cov  = vbParam2.invVhat[:,:,:,0].T / vbParam2.nuhat[:,np.newaxis, np.newaxis]
#     med_cov_det = np.median(np.linalg.det(cov))
#     peak = 1/(2* np.pi)**(cov.shape[-1]/2)/ med_cov_det
# #     fraction = 500/ pca_wf_all.shape[0]
#     fraction = 0.001
#     removal_peak = fraction*peak
#     mean = vbParam2.muhat[:, :, 0].T
#     diff = pca_wf_all[:,np.newaxis] - mean
#     prec = np.linalg.inv(cov)
#     maha = -np.matmul(np.matmul(diff[:,:,np.newaxis], prec), diff[:,:,:,np.newaxis]).squeeze(axis = (2,3))/2
#     prob = np.zeros(assignment2.shape[0])
    
#     for i in range(vbParam2.rhat.shape[1]):
#         idx = np.where(assignment2 == i)[0]
#         prob[idx] = np.exp(maha[idx, i]) * 1/ np.linalg.det(2*np.pi*cov[i])**0.5
#         idx2 = prob[idx]<removal_peak
# #         print(idx2.sum())
#         assignment2[idx[idx2]] = -1
#     idx_recovered = np.where(assignment2!=-1)[0]
#     if idx_recovered.size == 0:
...
#         idx = assignment2 == i
#         if idx.sum() == 0:
#             clust_to_remove.append(i)
#     vbParam2.rhat = np.delete(vbParam2.rhat, clust_to_remove, axis = 1)
#     vbParam2.rhat[idx_recovered] = vbParam2.rhat[idx_recovered]/np.sum(vbParam2.rhat[idx_recovered], axis = 1, keepdims = True)
#     vbParam2.muhat = np.delete(vbParam2.muhat, clust_to_remove, axis = 1)
#     vbParam2.Vhat = np.delete(vbParam2.Vhat, clust_to_remove, axis = 2)
#     vbParam2.invhat = np.delete(vbParam2.invVhat, clust_to_remove, axis = 2)
#     vbParam2.lambdahat = np.delete(vbParam2.lambdahat, clust_to_remove, axis = 0)
#     vbParam2.nuhat = np.delete(vbParam2.nuhat, clust_to_remove, axis = 0)
#     vbParam2.ahat = np.delete(vbParam2.ahat, clust_to_remove, axis = 0)
#     assignment2[idx_recovered] = np.argmax(vbParam2.rhat[idx_recovered], axis = 1)
        
#     print("time elapsed garbage collection = :", time.time() - start)
#     start = time.time()
#     eigenvalues, eigenvectors = np.linalg.eig(cov)
#     eigenvalues = eigenvalues[:,0]        
        
#     if plotting and gen<15:
#         split_type = 'pre_diffuse_cluster_removal'
#         plot_clustering_scatter(fig, 
#                     grid, x, gen,  
#                     assignment2[idx_recovered],
#                     pca_wf_all[idx_recovered], 
...
#         *************************************************        
#     '''
#     eigen_thresh = 3 * np.median(eigenvalues)
#     clusters_to_keep = np.where(eigenvalues<eigen_thresh)[0]
#     idx_recovered = idx_recovered[np.in1d(assignment2[idx_recovered], clusters_to_keep)]
#     if idx_recovered.size == 0:
#         return
    
#     for i, cluster in enumerate(clusters_to_keep):
        
#         idx  = assignment2[idx_recovered] == cluster
#         assignment2[idx_recovered[idx]] = i
    
    

#     vbParam2.Vhat = vbParam2.Vhat[:,:,clusters_to_keep]

#     vbParam2.invVhat = vbParam2.invVhat[:,:,clusters_to_keep]
#     vbParam2.muhat = vbParam2.muhat[:,clusters_to_keep]

#     vbParam2.nuhat = vbParam2.nuhat[clusters_to_keep]
#     vbParam2.lambdahat = vbParam2.lambdahat[clusters_to_keep]
#     vbParam2.ahat = vbParam2.ahat[clusters_to_keep]
#     vbParam2.rhat = vbParam2.rhat[:,clusters_to_keep]

#     vbParam2.rhat[idx_recovered] = vbParam2.rhat[idx_recovered]/ vbParam2.rhat[idx_recovered].sum(1, keepdims = True)
...
#         print(idx_recovered.size, idx_keep.size, assignment2.size)
    ...
    # Case #2: multiple clusters
        ...
        rhat = vbParam2.rhat[idx_recovered]
        rhat[rhat<0.1] = 0.0
        ...
        if verbose:
            print("chan "+str(channel)+' gen: '+str(gen) + 
                  " post annealing, stability " + str(np.round(stability2,2)) + 
                  " size: "+str(sizes2))
#         print(np.unique(assignment2), np.unique(assignment2[idx_recovered]))
...
#         print("time elapsed plotting= :", time.time() - start)
#         start = time.time()
            
            
            
        # if at least one stable cluster
        if np.any(stability2>mfm_threshold):
            
            # remove stable clusters 
            for clust in np.where(stability2>mfm_threshold)[0]:

                #print ("chan "+str(channel)+' gen: '+str(gen)+ " CASE #2: thresholded cluster")

                idx = np.where(assignment3==clust)[0]
                
                if wf[idx_keep][idx_recovered][idx].shape[0]<CONFIG.cluster.min_spikes: 
                    continue    # cluster too small
        ...
        # if all clusters are unstable: triage (also annealing is an option)
            ...
            if verbose:
                print("chan "+str(channel)+' gen: '+str(gen)+ 
                                " no stable clusters, connected_components_split "+
                                str(wf[idx_keep][idx_recovered].shape), stability2)
...
def ret_ellipse(feat1,feat2, vbPar, scale, color):
    ell = []
    cov = vbPar.invVhat[[feat1, feat2]][:, [feat1, feat2], :, 0] / vbPar.nuhat
    
    for i in range(vbPar.muhat.shape[1]):

        nstd = scale
        vals, vecs = eigsorted(cov[:,:,i].T)
        theta = np.degrees(np.arctan2(*-vecs[:, 0][::-1]))
        w, h = 2 * nstd * np.sqrt(vals)
        ellipse = Ellipse(xy=(vbPar.muhat[feat1,i,0], vbPar.muhat[feat2,i,0]),
                  width=w, height=h,
                  angle=theta, edgecolor = 'k', facecolor = color[i],fill = True, alpha = 0.5)
        ell += [ellipse]
    return ell
...
def garbage_collector(vbParam, pca_wf_all, CONFIG):
    ...
    if np.any(vbParam.nuhat>100):
        ...
        vbParam2, assignment2 = recover_spikes(vbParam2, pca_wf_all, CONFIG)
        idx_recovered = np.where(assignment2 != vbParam.rhat.shape[1])[0]
        assignment2[assignment2 == vbParam.rhat.shape[1]] = -1
        
    else:
        vbParam2, assignment2 = recover_spikes(vbParam, pca_wf_all, CONFIG)
        idx_recovered = np.arange(vbParam2.rhat.shape[0])
